...
app
  .run(function ($ionicPlatform, DatabaseValues, $rootScope, banco) {
    ...
    DatabaseValues.bancoDeDados.transaction(function (transacao) {
      ...
      banco.executa(
        "CREATE TABLE IF NOT EXISTS config (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `end_back` TEXT, `token` TEXT);"
      );
      banco.executa(
      ...
      banco.executa(
        "create table if not exists plots_lancamento_arv (id integer primary key autoincrement,plots_lancamento_id INTEGER, linha INTEGER, fim_linha TEXT, numero INTEGER, matrix TEXT, arvore INTEGER,dap FLOAT,altura FLOAT,dap2 FLOAT, alt_poda FLOAT, dominante TEXT, sync INTEGER DEFAULT 0, ult_arv_mult TEXT);"
      );

      try {
        var sql = `ALTER TABLE plots_lancamento_arv ADD ult_arv_mult TEXT `;
        var params = [];

        executaSQL(transacao, sql, params);
      } catch (e) {
        console.warn(
          "Essa coluna já foi adicionada: ult_arv_mult,tabela : plots_lancamento_arv"
        );
      }
    });
